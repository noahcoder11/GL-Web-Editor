        ...
        while(!tBuffer.done) {
            ...
            for(var a in vShaderObject._attributes) {
                ...
                for(var v = 0;v < current.length;v++){
                    ...
                    if(!vertices[v].args) {
                        vertexLength = vertices[v].length;
            ...
            for(var y = Math.ceil(vertices[0].y);y < Math.ceil(vertices[2].y);y++){
                ...
                for(var i in varying){
                    varyingY[i] = GL._interpolateY(vertices, varying[i], y, vertexLength > 2);
                }
                
                // Loop through x-values
                for(var x = start;x < end;x++){
                    var z = GL._interpolate(x, start, end, _int[1][0], _int[1][1])
                    var inv_z = vertexLength > 2 ? GL._interpolate(x, start, end, _int[2][0], _int[2][1]) : 1
