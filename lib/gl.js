class Sampler2D {
    constructor(texture) {
        this.data = GL.toImageData(texture)
...
class GL {
    ...
    static getProjection = function(FOV, a, zMin, zMax) {
        var ang = Math.tan(FOV * 0.5);
    ...
    static createShader = function(shader) {
        ...
        return new Function("params", shader)
    }
    ...
    static _interpolateY = function(vertices, values, y) {
        var out = [],
        ...
        for(var i = 0;i < values.step;i++){
            ...
            value1 = y < vertices[1][1] ? 
                GL._interpolate(y, vertices[0][1], vertices[1][1], v0[i], v1[i]) : 
                GL._interpolate(y, vertices[1][1], vertices[2][1], v1[i], v2[i])
            value2 = GL._interpolate(y, vertices[0][1], vertices[2][1], v0[i], v2[i])
    ...
    static render(tBuffer, fShaderObject, vShaderObject) {
        if(tBuffer.done) {
            tBuffer.reset()
        }
        
        // Get the pixels
        var pixels = GL.viewport.getImageData(0, 0, GL.viewport.canvas.clientWidth, GL.viewport.canvas.clientHeight).data
        ...
        while(!tBuffer.done) {
            ...
            for(var a in vShaderObject._attributes) {
                ...
                for(var v = 0;v < current.length;v++){
                    ...
                    vertices[v] = GL._shaders[0](object)
                }
            }
            ...
            for(var i in fShaderObject._varying){
                ...
                for(var t = 0;t < tri.length;t++){
                    data = data.concat(fShaderObject._varying[i].ref(tri[t]));
                }
            ...
            for(var y = Math.ceil(vertices[0][1]);y < Math.ceil(vertices[2][1]);y++){
                ...
                _x = GL._interpolateY(vertices, formatted, y),
                // get the sum of the values
                added = _x[0][0] + _x[0][1],
                // get the minimum x-value
                _min = Math.min(_x[0][0], _x[0][1]),
                ...
                end = ~~Math.min(_max, GL.viewport.canvas.clientWidth),
                ...
                for(var i in varying){
                    varyingY[i] = GL._interpolateY(vertices, varying[i], y);
                }
                ...
                for(var x = start;x < end;x++){
                    ...
                    for(var i in varyingY){
                        ...
                        for(var j = 0;j < varyingY[i].length;j++){
                            varyingX[i].push(GL._interpolate(x, start, end, varyingY[i][j][0], varyingY[i][j][1]));
                        }
                    }
                    ...
                    object._attributes = fShaderObject._attributes;
                    // Get the index of the current screen pixel
                    var idx = ~~x + ~~y * GL.viewport.canvas.clientWidth << 2,
                    // Get the pixel color from the fragment shader
                    pixel = GL._shaders[1](object)
        ...
        // Update the screen
        GL.viewport.putImageData(new ImageData(pixels, GL.viewport.canvas.clientWidth, GL.viewport.canvas.clientHeight), 0, 0)
    }
}